...
satisfy pr =
  character >>= \x ->
    depends (pure x) (unexpectedCharParser x) (pr x)
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser =
  foldRight (\p ps -> p >>= aggregate ps) $ pure Nil
...
surnameParser ::
  Parser Chars
surnameParser =
  (++) <$> (upper >>= aggregate (thisMany 5 lower)) <*> list lower
...
phoneParser ::
  Parser Chars
phoneParser =
  digit >>= aggregate phoneBodyParser >>= (is '#' >>) . return
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser b
  (<*>) =
    (. flmapParser) . flbindParser

aggregate :: Parser (List a) -> a -> Parser (List a)
aggregate ps x = (:.) <$> pure x <*> ps
